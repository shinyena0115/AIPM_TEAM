<template>
  ...
  <v-container>
      ...
      <v-card-text>
        ...
        >
          ...
          <div class="text-caption">
            {{ getDday(task.deadline) }} | {{ task.estimatedTime }}분 | 중요도:
            {{ task.importance }}
          </div>
        </div>
        <p class="text-caption text-grey mt-2">
          💡 AI가 마감일과 중요도를 고려하여 선정했습니다
        </p>
      </v-card-text>
    </v-card>
    ...
    <v-card v-if="analyzedTasks.length > 0" class="my-4">
      ...
      <v-card-text>
        <div
          v-for="(task, index) in analyzedTasks"
          :key="index"
          class="mb-4 pa-3 task-box"
        >
              ...
              <div class="d-flex justify-space-between align-center">
                <div>
                  ...
                  <p class="text-caption">
                    {{ getDday(task.deadline) }} | {{ task.estimatedTime }}분 |
                    {{ task.difficulty }} | 중요도: {{ task.importance }}
                  </p>
                </div>
    ...
    <v-card class="my-4">
      ...
      <v-card-text>
        <div v-if="tasks.length === 0">등록된 업무가 없습니다</div>
        ...
        <div v-if="incompleteTasks.length > 0">
          ...
          >
            <v-card-text>
              ...
              <p>
                소요시간: {{ task.estimatedTime }}분 | 난이도: {{ task.difficulty }} |
                유형: {{ task.taskType }}
              </p>
            </v-card-text>
            <v-card-actions>
...
export default {
  ...
  computed: {
    ...
    urgentTasks() {
      return this.incompleteTasks
        .filter(task => {
          const hoursLeft = this.getHoursLeft(task.deadline);
  ...
  methods: {
    ...
    async addTask() {
      ...
      const deadline = `${this.newTask.deadlineDate}T${this.newTask.deadlineTime}`;
    ...
    async getAIPriority() {
      ...
      try {
        const response = await axios.post('http://localhost:3000/api/tasks/ai-priority', {
    ...
    getDday(deadline) {
      const hoursLeft = this.getHoursLeft(deadline);

      if (hoursLeft < 0) return '마감 지남';
      if (hoursLeft < 1) return `${Math.floor(hoursLeft * 60)}분 남음`;
      if (hoursLeft < 24) return `${Math.floor(hoursLeft)}시간 남음`;
      return `D-${Math.floor(hoursLeft / 24)}`;
    },
    ...
    handleFileSelect(event) {
      ...
      newFiles.forEach(newFile => {
        ...
        if (!isDuplicate) this.selectedFiles.push(newFile);
      });
    ...
    async analyzeDocuments() {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          const failedCount = response.data.analyses.filter(a => a.error).length;

          this.selectedFiles = [];
          this.mergeMultiplePages = false;

          alert(
            `${this.analyzedTasks.length}개 파일 분석 완료!${
    ...
    async saveAllAnalyzedTasks() {
      ...
      for (const task of this.analyzedTasks) {
        const taskData = {
